...
import EditProductionCode from "./Edit";
...
const CodeModal = (props) => {
    ...
    const [target, setTarget] = useState(
        props.data.production_quantity * quantity
    );
    ...
    const onMenuClick = (e) => {
        setLoading(true);
        const key = props.data.random_id;
        if (e.key === "1") {
            setOpenEdit(Math.random());
        } else {
            axios
                .post("/delete_production_code", {
                    id: key,
                })
                .then((res) => {
                    setIsModalOpen(false);
                    setNotify(false);
                    setLoading(false);
                    message.success("Product code deleted!");
                    navigate(
                        location.pathname +
                            location.search +
                            "#" +
    ...
    return (
        <>
            {notify === "success" ? (
                <AppNotification
                    type="success"
                    message="Production has been added!"
                />
            ) : notify === "delete" ? (
                <AppNotification
                    type="success"
                ...
                <AppNotification
                    type="error"
                    message="Please input current quantity!"
                />
            ) : (
                ""
            )}
            {/* (props.data.random_id) */}
            <EditProductionCode open={openEdit} data={props.data} />
            <Dropdown.Button
                block
                size="medium"
                danger
            ...
            <Modal
                title={props.data.code_name}
                open={isModalOpen}
                onOk={handleOk}
                maskClosable={false}
                onCancel={handleCancel}
            >
                <b>Bread Name: {props.data.bread_name}</b>
                <br />
                <b>
                    Target quantity per kilo: {props.data.production_quantity}
                </b>
                <br />
                <b>Main Target: {props.data.production_quantity * quantity}</b>
                <br />
                Quantity
                <Input
                    type="number"
                    ...
                    <tbody>
                        {data.map((res) => (
                            <tr key={res.id + Math.random()}>
                                ...
                                <td>
                                    {res.bind === "Grams"
                                        ? (parseInt(res.quantity) * quantity) /
                                          1000
                                        : parseInt(res.quantity) * quantity}
                                    {res.bind === "Grams"
                                        ? "kg"
